...
| **本地 Excel** | `extract_excel.py` | ❌ 需手动补 | 通用方案，导入 .xlsx 后使用 |

> ⚠️ 各平台导出 xlsx 时**普遍丢失嵌入图片**。如需图片附件，优先使用 API 提取方式。
...
执行此 Skill 时，**严禁自行猜测以下信息**，必须停下来向用户确认：
...
1. **处理人 → 工号映射**
   ...
   - 生成 `handler_mapping` 写入 config.yaml
...
读取 `config.yaml` 获取：
- 列名映射（所有数据源共用 `feishu_columns` 配置块）
- DevOps 表单默认值
- 处理人 → 工号映射
- 对应数据源的 API 凭证
...
运行对应提取脚本，输出 `pending_defects.json`：
...
| 配置块 | 用途 |
|--------|------|
| `feishu_columns` | 列名映射（所有数据源共用） |
| `devops_defaults` | DevOps 表单默认值（项目名称、模块路径等） |
...
- **启动**：`node scripts/launch_cdp.mjs`，用系统 Chrome 暴露 CDP 端口 9222
- **登录态**：保存在 `.browser-profile/`，下次复用
- **会话过期**：操作超过 ~30 分钟触发"会话缓存丢失，请重新登录"
- **关联故事搜索 API 返回空**：`getLookUpList` 始终返回空数组，需用户提供故事编码直接设值
...
- **Python** >= 3.8（数据提取脚本）
- **Python 依赖**：`pip install -r requirements.txt`（requests, pyyaml, openpyxl, playwright）
- **Node.js 依赖**：`npm install`（playwright）
- **Chrome**：系统已安装 Google Chrome（CDP 方案）
